...
try:
    conn = mysql.connector.connect(
        host="192.168.1.13",   # My VM MaxScale IP address
        port=4006,
        user="maxuser",
        password="maxpwd",
    ...
    cursor.execute("SELECT MAX(Zip) FROM zipcodes_one;")
    for row in cursor:
    ...
    cursor.execute("""
            SELECT Zip, State, City, TotalWages FROM zipcodes_one WHERE State = 'KY'
            UNION
            SELECT Zip, State, City, TotalWages FROM zipcodes_two WHERE State = 'KY';
        """)
    for row in cursor:
        print(" ", row)
    print()
    ...
    cursor.execute("""
            SELECT Zip, State, City, TotalWages FROM zipcodes_one WHERE Zip BETWEEN 40000 AND 41000
            UNION
            SELECT Zip, State, City, TotalWages FROM zipcodes_two WHERE Zip BETWEEN 40000 AND 41000;
        """)
    for row in cursor:
        print(" ", row)
    print()

    print("TotalWages where state = 'PA':") # The totalWages if state = 'PA'
    cursor.execute("""
